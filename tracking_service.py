...
HTTP_HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; BZBets/2.2; +https://example.com)",
...
def _canonical_game_key(league: str, game_date: str, away_team: Optional[str], home_team: Optional[str]) -> str:
    return f"{league or 'UNKNOWN'}:{game_date or 'unknown'}:{_normalize_team(away_team)}:{_normalize_team(home_team)}"
...
def _read_tracking_rows(service=None, dedupe: bool = True) -> List[Dict[str, Any]]:
    ...
    for row in rows:
        ...
        if current is None:
            canonical[key] = row
            continue
        row_dt, cur_dt = _parse_dt(row.get("snapshot_at_ct")), _parse_dt(current.get("snapshot_at_ct"))
...
def record_prediction_snapshots(predictions: Iterable[Dict[str, Any]]) -> Dict[str, int]:
    ...
    if not eligible:
        return {"eligible": 0, "inserted": 0, "existing": 0, "line_updates": 0}
    ...
    try:
        ...
        for prediction in eligible:
            ...
            key = _canonical_game_key(
                prediction.get("sport") or "UNKNOWN", game_date,
                prediction.get("away_team_raw") or prediction.get("team1"),
                prediction.get("home_team_raw") or prediction.get("team2"),
            )
            ...
            if isinstance(game_time, datetime):
                ...
                game_time_ct = game_time_local.isoformat()
            ...
            if current is None:
                ...
                by_key[key] = {**dict(zip(HEADERS, row)), "_sheet_row": None, "_canonical_key": key}
                continue
            ...
            last_line = _to_float(current.get("last_market_total"))
            last_at = current.get("last_market_at_ct") or ""
            clv = _to_float(current.get("clv"))
            clv_status = current.get("clv_status") or ""

            if game_time_local and now_local < game_time_local:
                last_line = current_market
                last_at = now_local.isoformat()
                if first_line is not None:
                    clv = _clv_value(current.get("pick") or "", first_line, last_line)
                clv_status = "LAST_OBSERVED_PREGAME"
                line_updates += 1
            elif last_line is None and first_line is not None:
                last_line = first_line
                last_at = current.get("snapshot_at_ct") or ""
                clv = _clv_value(current.get("pick") or "", first_line, first_line)
                clv_status = "FIRST_OBSERVED_ONLY"

            if current.get("_sheet_row"):
                metadata_updates.append({
                    "range": f"'{TRACKING_SHEET}'!W{current['_sheet_row']}:AC{current['_sheet_row']}",
                    "values": [[model_version, last_line if last_line is not None else "", last_at,
                                clv if clv is not None else "", clv_status, key, snapshot_source]],
                })

        if append_rows:
            service.spreadsheets().values().append(
                spreadsheetId=_sheet_id(), range=f"'{TRACKING_SHEET}'!A:AC",
                valueInputOption="RAW", insertDataOption="INSERT_ROWS", body={"values": append_rows}
            ).execute()
        if metadata_updates:
            service.spreadsheets().values().batchUpdate(
                spreadsheetId=_sheet_id(), body={"valueInputOption": "RAW", "data": metadata_updates}
            ).execute()

        return {
            "eligible": len(eligible), "inserted": len(append_rows),
            "existing": len(eligible) - len(append_rows), "line_updates": line_updates,
            "duplicates_ignored": max(0, len(raw_rows) - len(canonical_rows)),
        }
    except Exception as exc:
        logging.exception("Prediction snapshot tracking failed: %s", exc)
        return {"eligible": len(eligible), "inserted": 0, "existing": 0, "line_updates": 0, "error": 1}
...
def _fetch_completed_games(league: str, game_date: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        response = requests.get(url, params={"dates": ymd, "limit": 1000}, headers=HTTP_HEADERS, timeout=15)
    ...
    for event in events:
        ...
        home_score, away_score = _score_from_competitor(home), _score_from_competitor(away)
        if not home_name or not away_name or home_score is None or away_score is None:
            continue
        completed.append({
            "home_team": home_name, "away_team": away_name, "home_score": home_score,
            "away_score": away_score, "actual_total": home_score + away_score,
        })
    return completed
    ...
    try:
        ...
        for row in pending:
            ...
            updates.append({
                ...
                "values": [[game["away_score"], game["home_score"], actual_total,
        ...
        if updates:
            service.spreadsheets().values().batchUpdate(
                spreadsheetId=_sheet_id(), body={"valueInputOption": "RAW", "data": updates}
            ).execute()
...
def _filter_state() -> Dict[str, str]:
    ...
    if window in {"7", "30", "season", "all"}: state["window"] = window
    if league in {"ALL", "MLB", "NFL", "NCAAF", "NBA"}: state["league"] = league
    if pick in {"ALL", "OVER", "UNDER"}: state["pick"] = pick
    if tier in {"ALL", "Strong Edge", "Good Edge", "Lean"}: state["tier"] = tier
    return state


def _row_in_filter(row: Dict[str, Any], filters: Dict[str, str]) -> bool:
    if filters["league"] != "ALL" and row.get("league") != filters["league"]: return False
    if filters["pick"] != "ALL" and row.get("pick") != filters["pick"]: return False
    if filters["tier"] != "ALL" and row.get("edge_tier") != filters["tier"]: return False
    try:
        game_date = datetime.strptime(row.get("game_date") or "", "%Y-%m-%d").date()
    except ValueError:
        return False
    today, window = datetime.now(LOCAL_TIMEZONE).date(), filters["window"]
    if window == "7" and game_date < today - timedelta(days=6): return False
    if window == "30" and game_date < today - timedelta(days=29): return False
    if window == "season" and game_date.year != today.year: return False
    return True


def _public_row(row: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "game_date": row.get("game_date") or "", "league": row.get("league") or "",
        "away_team": row.get("away_team") or "", "home_team": row.get("home_team") or "",
        "market_total": _to_float(row.get("market_total")),
        "last_market_total": _to_float(row.get("last_market_total")),
        "bz_total": _to_float(row.get("bz_total")), "edge": _to_float(row.get("edge")),
        "edge_tier": row.get("edge_tier") or "", "pick": row.get("pick") or "",
        "actual_total": _to_float(row.get("actual_total")), "result": row.get("result") or "",
        "status": row.get("status") or "", "clv": _to_float(row.get("clv")),
        "model_version": row.get("model_version") or "legacy_baseline",
    }


def _clv_summary(rows: Iterable[Dict[str, Any]]) -> Dict[str, Any]:
    values = [v for row in rows if row.get("pick") in {"OVER", "UNDER"}
              for v in [_to_float(row.get("clv"))] if v is not None]
    if not values:
        return {"count": 0, "average": 0.0, "positive_rate": 0.0}
    return {
        "count": len(values), "average": round(sum(values) / len(values), 2),
        "positive_rate": round(sum(1 for v in values if v > 0) / len(values) * 100.0, 1),
    }
...
def get_performance_dashboard() -> Dict[str, Any]:
    try:
        ...
        return {
            "overall": _aggregate([]), "by_league": [], "by_tier": [], "by_pick": [],
            "by_model_version": [], "recent": [], "tracked_games": 0, "total_tracked_games": 0,
            "pending_bets": 0, "graded_bets": 0, "pass_games": 0, "duplicates_ignored": 0,
            "clv": _clv_summary([]), "filters": _filter_state(), "error": str(exc),
        }

    filters = _filter_state()
    rows = [row for row in canonical_rows if _row_in_filter(row, filters)]
    graded_bets = [row for row in rows if row.get("pick") in {"OVER", "UNDER"}
                   and row.get("status") == "FINAL" and row.get("result") in {"WIN", "LOSS", "PUSH"}]
    pending_bets = sum(1 for row in rows if row.get("pick") in {"OVER", "UNDER"} and row.get("status") != "FINAL")

    def breakdown(field: str, labels: List[str]):
        out = []
        for label in labels:
            subset = [row for row in graded_bets if (row.get(field) or "legacy_baseline") == label]
            if subset:
                out.append({"label": label, **_aggregate(subset)})
        return out

    versions = sorted({row.get("model_version") or "legacy_baseline" for row in graded_bets})
    recent = sorted((_public_row(row) for row in rows if row.get("status") == "FINAL"),
                    key=lambda row: row.get("game_date") or "", reverse=True)[:50]
    ...
    return {
        ...
        "recent": recent, "tracked_games": len(rows), "total_tracked_games": len(canonical_rows),
        "graded_bets": len(graded_bets), "pending_bets": pending_bets,
        "pass_games": sum(1 for row in rows if row.get("pick") == "PASS"),
        "duplicates_ignored": max(0, len(raw_rows) - len(canonical_rows)),
        "clv": _clv_summary(rows), "filters": filters, "model_version": MODEL_VERSION,
    }
